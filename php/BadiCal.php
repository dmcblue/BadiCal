<?php
	class BadiCal extends BadiBase{
		...
		static public function getNawruzDate($gYear){
			return new DateTime(
				$gYear.'-'
				.'03-' 
				.self::getNawruz(new BadiDate($gYear, 5, 1)));
		}
		...
		static public function getAyyamiha($bYear){
			...
			$this_nr =
				new DateTime(
					$gYear.'-' 
					.'03-' 
					.self::getNawruz(new BadiDate($gYear, 4, 1))
				);
		...
		static public function GregorianToBadi($one, $two, $three, $startOf = false){
			...
			$gDate = new BadiDate($one, $two, $three);
		...
		static public function BadiToGregorian($one, $two, $three, $startsOn = false){
			$bDate = new BadiDate($one, $two, $three);
			...
			return new BadiDate($date->format('Y'), $date->format('n'), $date->format('j'), self::GREGORIAN);
		}
	}